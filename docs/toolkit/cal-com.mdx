...
Create a standard business hours schedule:
...
Update a schedule with date-specific overrides for holidays or special events:
...
The integration will return error messages from Cal.com API if:

- Invalid API key or authentication fails
- Schedule ID does not exist
- Invalid time zone specified
- Malformed availability or override data
- Time conflicts in scheduling

## Time Zone Reference

Common time zone IDs supported:
...
For a complete list, see [IANA Time Zone Database](https://www.iana.org/assignments/timezone-abbr/timezone-abbr.xhtml).
