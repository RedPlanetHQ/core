...
Notion supports various property types that the integration handles:
...
### Date Properties
Date properties are split into components:
- `date:{property}:start` - Start date
- `date:{property}:end` - Optional end date
- `date:{property}:is_datetime` - Whether it includes time (0 or 1)

Example: `"date:Timeline:start": "2024-02-15"`

### Specialized Properties
- **Place Properties**: Address, coordinates, and location data
  - `place:{property}:name` - Location name
  - `place:{property}:address` - Street address
  - `place:{property}:latitude` - Latitude coordinate
  - `place:{property}:longitude` - Longitude coordinate
- **Files**: Attachments and file links
- **Relations**: References to other database items
- **Rollups**: Aggregated data from related items

### Special Property Names

Properties named "id" or "url" (case insensitive) must be prefixed with "userDefined:":
- Use `userDefined:id` instead of `id`
- Use `userDefined:URL` instead of `url`

## Block Types Supported

The integration handles various Notion block types:

- **Text Blocks**: Paragraphs, headings (h1, h2, h3), quotes, callouts
- **Lists**: Bulleted lists, numbered lists, to-do items
- **Content**: Code blocks, equations, dividers
- **Media**: Images, bookmarks, embeds, videos
- **Organization**: Toggle blocks, child pages, child databases
- **Rich Text**: Full markdown formatting support

## Connection Details

- **API Version**: Notion API v1 (2022-06-28)
- **Base URL**: https://api.notion.com/v1
- **Rate Limits**: Standard Notion API rate limits apply
