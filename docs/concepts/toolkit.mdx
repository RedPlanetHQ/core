...
The Toolkit is CORE's hands. While the Meta Agent thinks and Memory remembers, the Toolkit actually *does things* — creates issues, drafts emails, schedules meetings, sends messages — across all your connected apps.
...
Toolkit actions are exposed to your AI agents through **MCP (Model Context Protocol)**. CORE provides a single MCP endpoint that bundles all your integrations:

```
https://mcp.getcore.me/api/v1/mcp
```

Instead of configuring a separate MCP server for each app (one for GitHub, one for Slack, one for Calendar...), you connect CORE once and get everything through a single endpoint.

This means:
- **Claude Code** can draft emails and create issues without separate MCP servers
- **Cursor** can check your calendar and search Slack without additional setup
- **Any MCP-compatible tool** gets access to your full toolkit

Learn more in the [MCP documentation](/mcp/overview).
