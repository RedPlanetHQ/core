...
The gateway can be deployed three ways. Pick the one that fits where you want it to run.

<Tabs>
  <Tab title="Native">
    Best for running the gateway on your own laptop or workstation, with full access to your local browser, shells, and folders.
  ...
  <Tab title="Docker">
    Best for self-hosted servers, Fly, Render, or any Docker host you control. The bundled image ships Claude Code, Codex CLI, Playwright Chromium, and pre-registers `/app` as a coding workspace.

    The repo includes a ready compose file under [`hosting/gateway`](https://github.com/RedPlanetHQ/core/tree/main/hosting/gateway):

    ```bash
    cd hosting/gateway
    cp .env.example .env       # fill in COREBRAIN_API_KEY (and the agent token below)
    docker compose up -d
    docker compose logs -f corebrain-gateway   # grab the auto-generated security key on first boot
    ```

    Set the token for whichever coding agent you plan to use — the container runs both `claude-code` and `codex-cli`, but each needs its own credential to skip OAuth:

    - **`CLAUDE_CODE_OAUTH_TOKEN`** — long-lived token from `claude setup-token` (run on a laptop where you've already done `claude login`). See [Claude Code authentication](https://docs.claude.com/en/docs/claude-code/setup#authentication).
    - **`OPENAI_API_KEY`** — standard OpenAI API key from the [API Keys dashboard](https://platform.openai.com/api-keys). See [Codex CLI auth](https://github.com/openai/codex#authentication).

    If you leave `COREBRAIN_GATEWAY_SECURITY_KEY` blank in `.env`, the gateway generates one on first boot and prints it to the logs — copy it (with the gateway URL) into the webapp's **Register gateway** dialog.
  </Tab>

  <Tab title="Railway">
    Best for a fully managed cloud gateway with one-click setup. Railway provisions the container, persistent volumes, and a public URL for you.

    [![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/core-gateway)

    <Steps>
      <Step title="Click Deploy">
        Use the button above to launch the template on Railway.
      </Step>

      <Step title="Set environment variables">
        Fill in the required variables when prompted:
        ...
        See the full list under [Environment Variables](#environment-variables-headless--docker).
      </Step>

      <Step title="Grab the security key">
        If you didn't set `COREBRAIN_GATEWAY_SECURITY_KEY`, open the deploy logs once the service is up and copy the auto-generated key printed on first boot.
      </Step>

      <Step title="Register with the webapp">
        Copy the public Railway URL and paste it (with the security key) into **Settings → Gateways → New gateway**.
      </Step>
    </Steps>
  </Tab>
</Tabs>
