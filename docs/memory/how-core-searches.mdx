...
flowchart LR
    ...
    AGENT["Agent uses context\nto ground its response"]

    QUERY --> DECOMP --> search --> RANK --> CONTEXT --> AGENT
```

When you ask the Butler a question or search your memory:

<Steps>
  <Step title="Query Classification">
    CORE analyzes your query to determine the [query type](/memory/query_types) (aspect, entity, temporal, exploratory, or relationship), relevant time range, and which aspects to prioritize.
  </Step>
...
<CardGroup cols={3}>
  <Card title="Query Types" icon="magnifying-glass" href="/memory/query_types">
    The 5 query types and how CORE classifies your questions
  </Card>

  <Card title="How CORE Ingests" icon="download" href="/memory/how-core-ingests">
