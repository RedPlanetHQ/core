...
<Steps>
  <Step title="Raw Input Capture">
    Information arrives from conversations with the Butler, integration activity (GitHub commits, Slack messages, Linear issues), manual document uploads, or scheduled syncs from connected apps.
  </Step>
  ...
  <Step title="Statement Extraction">
    Facts are extracted as structured triples: subject → predicate → object. Each statement is classified into one of [11 aspects](/memory/aspects) (Identity, Preference, Decision, etc.).
  </Step>
...
**Automatic:** Connected integrations feed activity into memory in real-time or via scheduled syncs. Conversations with the Butler are captured automatically. Zero effort - happens in the background.
...
<CardGroup cols={3}>
  ...
  <Card title="Statement Aspects" icon="tags" href="/memory/aspects">
    How facts are categorized during ingestion
  </Card>

  <Card title="Labels" icon="tag" href="/memory/labels">
