...
The fastest path is the CLI wizard — one command, no git clone:

```bash
npm install -g @redplanethq/corebrain && corebrain setup
```

Or, for a one-click cloud deployment, use Railway:
...
See the [setup guide](/self-hosting/setup) for the full walkthrough.
