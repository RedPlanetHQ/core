...
If you have the CLI installed, the fastest path is:
...
The wizard prompts for an install directory, an AI provider (OpenAI / Anthropic / Google / Ollama / Azure / OpenRouter / DeepSeek / Vercel / Groq / Mistral / xAI), the relevant API key, optional `OPENAI_BASE_URL` for OpenAI-compatible proxies, and a chat model. It auto-generates `SESSION_SECRET`, `ENCRYPTION_KEY`, `MAGIC_LINK_SECRET`, and the Neo4j password, fetches the canonical `docker-compose.yaml` + `.env.example` from `main`, applies your overrides, and runs `docker compose up -d`.
