...
async function createMCPClient(accessToken: string): Promise<Client> {
  const client = new Client({
    name: "swiggy-dineout-integration",
    version: "1.0.0",
  });

  const transport = new StreamableHTTPClientTransport(new URL(SWIGGY_DINEOUT_MCP_URL), {
    requestInit: {
      headers: {
...
export async function getTools(config?: Record<string, string>): Promise<MCPTool[]> {
  ...
  try {
    ...
    console.error("Error fetching tools from Swiggy Dineout MCP:", error.message);
    return [];
  } finally {
    if (client) {
      await client.close();
