    ...
    var Command2 = class _Command extends EventEmitter2 {
      ...
      constructor(name) {
        ...
        this._outputConfiguration = {
          ...
          getOutHasColors: () => {
            var _a5, _b;
            return useColor() ?? (process2.stdout.isTTY && ((_b = (_a5 = process2.stdout).hasColors) == null ? void 0 : _b.call(_a5)));
          },
          getErrHasColors: () => {
            var _a5, _b;
            return useColor() ?? (process2.stderr.isTTY && ((_b = (_a5 = process2.stderr).hasColors) == null ? void 0 : _b.call(_a5)));
          },
 ...
 - ${executableDirMessage}`;
      ...
      _dispatchHelpCommand(subcommandName) {
        ...
        return this._dispatchSubcommand(
          subcommandName,
          [],
          [((_a5 = this._getHelpOption()) == null ? void 0 : _a5.long) ?? ((_b = this._getHelpOption()) == null ? void 0 : _b.short) ?? "--help"]
        );
      }
      /**
...
var textEncoder = typeof TextEncoder === "function" ? new TextEncoder() : new util2__default.default.TextEncoder();
var CRLF = "\r\n";
  ...
  getSession(authority, options) {
    ...
    if (authoritySessions) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (!sessionHandle.destroyed && !sessionHandle.closed && util2__default.default.isDeepStrictEqual(sessionOptions, options)) {
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (utils_default.isSpecCompliantForm(data)) {
      ...
      if (!headers.hasContentLength()) {
        try {
          const knownLength = await util2__default.default.promisify(data.getLength).call(data);
...
async function addTagToContact(apiKey, contactId, tagName) {
  return systemeRequest("POST", `/contacts/${contactId}/tags`, apiKey, { name: tagName });
}
async function getSales(apiKey, page = 1) {
  return systemeRequest("GET", `/sales?page=${page}`, apiKey);
}
async function getFunnels(apiKey, page = 1) {
  return systemeRequest("GET", `/funnels?page=${page}`, apiKey);
}
async function grantCourseAccess(apiKey, courseId, studentEmail) {
...
async function createServer(apiToken, config) {
  return hetznerRequest("POST", "/servers", apiToken, config);
}
async function getServer(apiToken, serverId) {
  return hetznerRequest("GET", `/servers/${serverId}`, apiToken);
}
async function deleteServer(apiToken, serverId) {
  return hetznerRequest("DELETE", `/servers/${serverId}`, apiToken);
}
function generateCloudInitScript(params) {
  return `#!/bin/bash
set -euo pipefail
...
async function provisionCustomerServer(params) {
  var _a5, _b, _c, _d;
  ...
  const server = await createServer(params.hetznerApiToken, {
    name: serverName,
    server_type: params.serverType || "cpx31",
    // 4 vCPU, 8GB RAM - good for KI
    image: "ubuntu-24.04",
  ...
  const serverIp = ((_c = (_b = (_a5 = server.server) == null ? void 0 : _a5.public_net) == null ? void 0 : _b.ipv4) == null ? void 0 : _c.ip) || "pending";
  try {
  ...
  return {
    serverId: (_d = server.server) == null ? void 0 : _d.id,
    serverName,
    serverIp,
    adminPassword,
    status: "provisioning",
...
// src/schedule.ts
function createActivityMessage(text, sourceURL = "") {
  return {
    type: "activity",
    data: { text, sourceURL }
  };
...
async function handleSchedule(config, state) {
  ...
  try {
    ...
    const messages = [];
    const provisionedCustomers = JSON.parse(settings.provisionedCustomers || "[]");
    try {
      const salesData = await getSales(config.systeme_api_key);
      const sales = (salesData == null ? void 0 : salesData.items) || [];
      for (const sale of sales) {
        const saleId = String(sale.id);
        if (settings.lastSaleId && Number(saleId) <= Number(settings.lastSaleId)) {
          continue;
        }
        const customerEmail = sale.contactEmail || ((_a5 = sale.contact) == null ? void 0 : _a5.email);
        const customerName = sale.contactName || ((_b = sale.contact) == null ? void 0 : _b.firstName) || "Kunde";
        const amount = sale.amount || sale.planAmount || 99;
        messages.push(
          createActivityMessage(
            `Neuer Verkauf: ${customerName} (${customerEmail}) hat das KI-Power Abo f\xFCr \u20AC${amount}/Monat gebucht!`,
        ...
        if (!provisionedCustomers.includes(customerEmail)) {
          try {
            ...
            messages.push(
              createActivityMessage(
                `Server automatisch bereitgestellt f\xFCr ${customerName}: IP ${result.serverIp} | Open WebUI: ${result.accessUrls.openWebUI} | n8n: ${result.accessUrls.n8n}`,
            ...
            messages.push(
              createActivityMessage(
                `FEHLER: Server-Provisioning fehlgeschlagen f\xFCr ${customerEmail}: ${error.message}`
              )
            );
          }
        }
    ...
    try {
      ...
      const servers = (serversData == null ? void 0 : serversData.servers) || [];
      const managedServers = servers.filter(
        (s) => {
          var _a6;
          return ((_a6 = s.labels) == null ? void 0 : _a6.managed_by) === "ki-fastfood-system";
        }
      );
      const activeCount = managedServers.filter((s) => s.status === "running").length;
    ...
    try {
      ...
      const contacts = (contactsData == null ? void 0 : contactsData.items) || [];
      const lastSync = settings.lastSyncTime ? new Date(settings.lastSyncTime) : new Date(Date.now() - 24 * 60 * 60 * 1e3);
  ...
  "~validate"(data) {
    ...
    if (!this["~standard"].async) {
      try {
        ...
        if ((_b = (_a5 = err == null ? void 0 : err.message) == null ? void 0 : _a5.toLowerCase()) == null ? void 0 : _b.includes("encountered")) {
  ...
  strict(message) {
    ...
    return new _ZodObject({
      ...
      ...message !== void 0 ? {
        errorMap: (issue, ctx) => {
          var _a5, _b;
          const defaultError = ((_b = (_a5 = this._def).errorMap) == null ? void 0 : _b.call(_a5, issue, ctx).message) ?? ctx.defaultError;
...
function parseArrayDef(def, refs) {
  ...
  if (((_a5 = def.type) == null ? void 0 : _a5._def) && ((_c = (_b = def.type) == null ? void 0 : _b._def) == null ? void 0 : _c.typeName) !== ZodFirstPartyTypeKind.ZodAny) {
...
function addFormat(schema, value, message, refs) {
  var _a5;
  if (schema.format || ((_a5 = schema.anyOf) == null ? void 0 : _a5.some((x) => x.format))) {
    if (!schema.anyOf) {
      schema.anyOf = [];
    }
    if (schema.format) {
...
function addPattern(schema, regex, message, refs) {
  var _a5;
  if (schema.pattern || ((_a5 = schema.allOf) == null ? void 0 : _a5.some((x) => x.pattern))) {
    if (!schema.allOf) {
      schema.allOf = [];
    }
    if (schema.pattern) {
      ...
      if (inCharGroup) {
        if (source[i].match(/[a-z]/)) {
          if (inCharRange) {
            ...
            inCharRange = false;
          } else if (source[i + 1] === "-" && ((_a5 = source[i + 2]) == null ? void 0 : _a5.match(/[a-z]/))) {
            pattern += source[i];
            inCharRange = true;
          } else {
...
function parseRecordDef(def, refs) {
  ...
  if (refs.target === "openApi3" && ((_a5 = def.keyType) == null ? void 0 : _a5._def.typeName) === ZodFirstPartyTypeKind.ZodEnum) {
    return {
      type: "object",
...
var parseOptionalDef = (def, refs) => {
  var _a5;
  if (refs.currentPath.toString() === ((_a5 = refs.propertyPath) == null ? void 0 : _a5.toString())) {
    return parseDef(def.innerType._def, refs);
  }
...
function parseDef(def, refs, forceResolution = false) {
  ...
  if (refs.override) {
    const overrideResult = (_a5 = refs.override) == null ? void 0 : _a5.call(refs, def, refs, seenItem, forceResolution);
...
// src/mcp/index.ts
var ListContactsSchema = external_exports.object({
  page: external_exports.number().optional().default(1).describe("Page number"),
  limit: external_exports.number().optional().default(20).describe("Items per page")
});
var CreateContactSchema = external_exports.object({
  email: external_exports.string().email().describe("Contact email address"),
  firstName: external_exports.string().optional().describe("First name"),
  lastName: external_exports.string().optional().describe("Last name"),
  tags: external_exports.array(external_exports.string()).optional().describe("Tags to apply")
});
var TagContactSchema = external_exports.object({
  contactId: external_exports.string().describe("Systeme.io contact ID"),
  tagName: external_exports.string().describe("Tag name to add")
});
var ListSalesSchema = external_exports.object({
  page: external_exports.number().optional().default(1).describe("Page number")
});
var ListFunnelsSchema = external_exports.object({
  page: external_exports.number().optional().default(1).describe("Page number")
});
var ListServersSchema = external_exports.object({});
var GetServerSchema = external_exports.object({
  serverId: external_exports.string().describe("Hetzner server ID")
});
var ProvisionServerSchema = external_exports.object({
  customerEmail: external_exports.string().email().describe("Customer email for the new server"),
  customerName: external_exports.string().describe("Customer name"),
  serverType: external_exports.string().optional().default("cpx31").describe("Hetzner server type (e.g., cpx31, cpx41)"),
  location: external_exports.string().optional().default("nbg1").describe("Hetzner datacenter (nbg1, fsn1, hel1)")
});
var DeleteServerSchema = external_exports.object({
  serverId: external_exports.string().describe("Hetzner server ID to delete")
});
var GrantCourseAccessSchema = external_exports.object({
  courseId: external_exports.string().describe("Systeme.io course ID"),
  studentEmail: external_exports.string().email().describe("Student email to grant access")
});
var GetRevenueReportSchema = external_exports.object({});
  ...
  return [
    {
      ...
      data: [
        ...
        {
          name: "tag_contact",
          description: "F\xFCge einem Kontakt in Systeme.io einen Tag hinzu",
          inputSchema: zodToJsonSchema(TagContactSchema)
        },
        {
          name: "list_sales",
          description: "Liste alle Verk\xE4ufe/Bestellungen aus Systeme.io auf",
        ...
        },
        {
          ...
          description: "Stelle automatisch einen neuen KI-Power Server f\xFCr einen Kunden bereit (Hetzner + Cloud-Init + Docker Stack)",
          inputSchema: zodToJsonSchema(ProvisionServerSchema)
        },
        {
          name: "delete_server",
          description: "L\xF6sche einen Hetzner Server (z.B. bei K\xFCndigung)",
...
async function callTool(name, args, config) {
  ...
  try {
    switch (name) {
      ...
      case "create_contact": {
        const tags = (_a5 = args.tags) == null ? void 0 : _a5.map((t) => ({ name: t }));
      ...
      case "delete_server": {
        ...
        return [{ type: "tool_result", data: { content: `Server ${args.serverId} gel\xF6scht. ${JSON.stringify(result)}` } }];
      }
      // --- BUSINESS ---
      case "revenue_report": {
        const serversData = await listServers(hetznerToken);
        const servers = (serversData == null ? void 0 : serversData.servers) || [];
        const managed = servers.filter((s) => {
          var _a6;
          return ((_a6 = s.labels) == null ? void 0 : _a6.managed_by) === "ki-fastfood-system";
        });
        const active = managed.filter((s) => s.status === "running");
        const report = {
          ...
          servers: managed.map((s) => {
            var _a6, _b, _c;
            return {
              name: s.name,
              status: s.status,
              ip: (_b = (_a6 = s.public_net) == null ? void 0 : _a6.ipv4) == null ? void 0 : _b.ip,
              customer: (_c = s.labels) == null ? void 0 : _c.customer_email,
              created: s.created
            };
          })
        };
...
var SystemeioHetznerCLI = class extends IntegrationCLI {
  constructor() {
  ...
  async getSpec() {
    return {
      ...
      description: "Vollautomatisches KI-Business System: Systeme.io Affiliate Marketing + Hetzner Server Auto-Provisioning. Verkauft KI-Server Abos f\xFCr \u20AC99/Monat, provisioniert automatisch Server mit Open WebUI, n8n und vollem KI-Stack. Marketing \u2192 Akquisition \u2192 Verkauf \u2192 Einrichtung \u2192 Abrechnung - alles automatisch.",
      icon: "server",
      category: "business-automation",
      schedule: {
        frequency: "*/10 * * * *"
        // Check for new sales every 10 minutes
      },
      mcp: {
        type: "cli"
      },
