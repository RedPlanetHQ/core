...
`
    : `
PREP RULES:
0. CHECK INPUT SHAPE FIRST. Read the task description and decide what the user gave you (see \`<doing_tasks>\` > Input shape in your system prompt):
   ...
   - If the description is a GOAL (a desired outcome — you need to figure out the steps):
     → Apply the Complexity rules from \`<doing_tasks>\`.
...
1. Run the readiness check (see \`<readiness_check>\`). Load the appropriate skill from \`<skills>\`:
