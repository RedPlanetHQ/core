/**
 * Conversation normalization prompt, decomposed into named sections.
 *
 * These constants hold the ORIGINAL template text verbatim, extracted by pure
 * string slicing rather than retyped, so the hosted profile can reassemble a
 * byte-identical prompt. `normalize-golden.test.ts` proves that property and
 * will fail loudly if any section, separator, or ordering is edited.
 *
 * Sections join with a blank line ("\n\n"), and the assembled system prompt
 * ends with a single trailing newline, exactly as the original template did.
 *
 * The two dynamic sections (entity_types, ingestion_rules) are functions
 * because they interpolate request context; the rest are static.
 */

export const PREAMBLE_SECTION = `You are C.O.R.E. (Contextual Observation & Recall Engine), a smart memory enrichment system.
...
export const SPEAKER_ROLE_AWARENESS_SECTION = `<speaker_role_awareness>
...
export const ENRICHMENT_STRATEGY_SECTION = `<enrichment_strategy>
...
export const TEMPORAL_RESOLUTION_SECTION = `<temporal_resolution>
...
export const VISUAL_CONTENT_CAPTURE_SECTION = `<visual_content_capture>
...
export const STRATEGIC_ENRICHMENT_SECTION = `<strategic_enrichment>
...
export const QUALITY_CONTROL_SECTION = `<quality_control>
...
export const ENRICHMENT_EXAMPLES_SECTION = `<enrichment_examples>
...
export const OUTPUT_FORMAT_SECTION = `CRITICAL OUTPUT FORMAT REQUIREMENT:
...
export function buildEntityTypesSection(entityTypes: string): string {
  return `<entity_types>
${entityTypes}
</entity_types>`;
}

export function buildIngestionRulesSection(
  ingestionRules: string | undefined,
  source: string,
): string {
  return `<ingestion_rules>
${
  ingestionRules
    ? `Apply these rules for content from ${source}:
${ingestionRules}
